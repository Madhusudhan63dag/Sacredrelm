...
const ProductPage = () => {
    ...
    // Compute site URL (can be overridden via env at build time)
    const SITE_URL = process.env.REACT_APP_SITE_URL || 'https://sacredrelm.com';
    ...
    const productSchema = {
        ...
        "offers": {
            "@type": "Offer",
            "url": product ? `${SITE_URL}${createProductUrl(product.id, product.name)}` : window.location.href,
            "priceCurrency": "INR",
    ...
    return (
        <>
            ...
            {/* SEO Optimization */}
            <Helmet>
                <title>{product.name} | Sacred Relm</title>
                <meta name="description" content={`Buy ${product.name} - ${product.description?.substring(0, 150) || 'Premium spiritual product at the best price'}`} />
                <meta name="keywords" content={`${product.name}, ${product.category}, spiritual items, sacred, buy online`} />
                <link rel="canonical" href={`${SITE_URL}${createProductUrl(product.id, product.name)}`} />
                <meta property="og:title" content={`${product.name} | Sacred Relm`} />
                <meta property="og:description" content={product.description?.substring(0, 150) || 'Premium spiritual product at the best price'} />
                <meta property="og:image" content={`${SITE_URL}${product.photo.image1}`} />
                <meta property="og:url" content={`${SITE_URL}${createProductUrl(product.id, product.name)}`} />
                        ...
                        <div className="bg-gradient-to-r from-orange-600 to-purple-600 text-white px-2 sm:px-4 py-2 text-center shadow-lg">
                            <div className="container mx-auto flex items-center justify-center gap-1 sm:gap-2">
                                ...
                                <p className="text-xs sm:text-sm font-medium">
                                    <span className="font-bold">EXCLUSIVE:</span> Free accessories worth ₹{Math.floor(product.cost * 0.2)}
                                </p>
                            </div>
                        </div>
                    </div>
